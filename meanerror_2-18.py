...
a1 = model_before.coef_[2]  # coefficient for x²
b1 = model_before.coef_[1]  # coefficient for x
c1 = model_before.intercept_  # constant term

a2 = model_after.coef_[2]
b2 = model_after.coef_[1]
c2 = model_after.intercept_

# Find intersection by solving a1x² + b1x + c1 = a2x² + b2x + c2
# Rearrange to: (a1-a2)x² + (b1-b2)x + (c1-c2) = 0
a = a1 - a2
b = b1 - b2
c = c1 - c2
...
if discriminant >= 0:
    ...
    plt.plot(x_intersection, y_intersection, 'go', markersize=12, label='Curve Intersection (Bounce Point)')
